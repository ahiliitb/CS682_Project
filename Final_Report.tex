...
src/main/resources/
  ...
  templates/
  static/data/
scripts/selenium_iitb/
\end{lstlisting}

\subsection{Implemented Key Modules}
\textbf{Authentication and Access Control}
\begin{itemize}[leftmargin=*]
    \item IITB email-based registration and verification.
    \item SSO-style one-time code login flow with role synchronization.
    \item Admin-restricted routes for moderation and statistics.
\end{itemize}

\textbf{Marketplace and Exchange}
\begin{itemize}[leftmargin=*]
    \item Buy/sell with item photo uploads, suggested pricing, and transaction-linked ratings.
...
\begin{itemize}[leftmargin=*]
    \item Team module with team-size restrictions.
...
Nitesh Singh (22b0919) & Dashboard and auth-flow improvements, SSO code flow, role-control integration, moderation and notifications integration, suggested price completion, admin-only statistics restriction, and team-size restriction implementation. \\
